...
### 3.3 Install KDE Recommends skipped by MX Linux

MX Linux sets `APT::Install-Recommends "false"` by default. On vanilla Debian, `apt install kde-full` also installs all `Recommends` dependencies — which is where the majority of Plasma integration packages live (`kscreen`, `powerdevil`, `plasma-nm`, `kwin-x11`, etc.). On MX Linux, these are silently skipped, leaving a desktop that looks installed but is missing critical functionality.

The install block in 3.2 covers most of these. The following catches the remaining KDE-specific Recommends that are not in that block:
...
> **How the definitive list was generated:** `apt depends` was run against every KDE metapackage (`kde-plasma-desktop`, `kde-standard`, `kde-full`, `task-kde-desktop`, and all sub-metapackages) plus `plasma-desktop` and `plasma-workspace` directly. All `Depends` and `Recommends` were collected, deduplicated, and checked against installed packages. Non-KDE recommends (gimp, libreoffice plugins, accessibility tools, desktop toys, dev tools, input method data) were excluded. The audit script and raw results are in this repo.
...
Even after installing `kde-full`, the following required manual installation. **Root cause:** MX Linux sets `APT::Install-Recommends "false"`, and Debian classifies most KDE integration packages as `Recommends`, not `Depends`. See [10.9](#109-mx-linux-skips-most-kde-integration-packages-silently) for details.

- `kwin-x11` (not pulled in by default — only `kwin-wayland` is; required for X11/XRDP sessions)
- `kscreen` (display configuration — cannot change resolution/arrangement without it)
...
- `print-manager`, `kinfocenter`, `khelpcenter`, `kmix` (standard KDE utilities)
- `ksshaskpass`, `kde-config-gtk-style`, `svgpart` (integration components)
...
### 10.9 MX Linux skips most KDE integration packages silently

MX Linux ships with `APT::Install-Recommends "false"` in its apt configuration. This is by design and should not be changed — but it has a major impact on KDE installation.

In Debian's KDE packaging, critical integration packages are classified as `Recommends`, not `Depends`. This includes `kscreen`, `kwin-x11`, `powerdevil`, `plasma-nm`, `plasma-pa`, `bluedevil`, `kmenuedit`, and dozens more. On vanilla Debian (where recommends are installed by default), `sudo apt install kde-full` produces a fully functional desktop. On MX Linux, the same command produces a desktop that appears installed but is missing battery management, bluetooth, network tray icons, display configuration, and other core functionality.

The solution is not to enable recommends globally — it's to explicitly install the missing KDE-specific packages. Sections 3.2 and 3.3 of this guide contain the definitive list, generated by auditing `apt depends` output against every KDE metapackage.

To verify your apt recommends setting:

```bash
clear

grep -r "Install-Recommends" /etc/apt/apt.conf* 2>/dev/null
```
...
- [x] All supplementary packages installed (Phase 1, section 3.2)
- [x] KDE-specific Recommends installed (Phase 1, section 3.3 — required on MX Linux)
